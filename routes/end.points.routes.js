...
router.post('/update-auto', async (req, res) => {
    console.log("Este es el request para editar autos: ", req.body);
    // let body = req.body;

    const bodyRequest = req.body;
    let carFound = await carSchema.findOne({ _id: bodyRequest.id });

    if(!carFound){
        console.log("No encontrado");
        res.body({ errorMessage:"Id not found in database"});
    } else {
        console.log("Encontrado:",carFound);
const updateCar = await carFound.updateOne(bodyRequest);
